...
Timer kpTimer(100, scan16key);                          // Software Timer to scan every 100ms = Run the function scan16key()
...
if (strcmp(OP5_A7, "LASERRCV") == 0)
{
  ...
  if ((millis()-getLaserLastTime) > getLaserInterval) // => It's time to check the LDR signal
  {
    ...
    // If door light ON time is expired, temporarily disable laser and turn CAB lights OFF. (Door left open!)
    if ((millis()-DOORlightOnTime) >= MaxDOORlightOnTime && DOORmovstatus != "Laser monitored doors closed") // Doors seem to be open and time is up!
    {
      DOORlightsOFF();
