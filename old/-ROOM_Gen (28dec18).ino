...
// - A5 Option: PIR2 movement sensor: Triggers a second group of lights
...
int DOORLigtOnLevel = 200; // Over this laser level: Lights turn OFF
...
void loop()
{
  ...
  if ((millis()-HomebridgeLastTime) > HomebridgeInterval)
  {
    ...
    if (strcmp(GAS_A2D7, "DUST") == 0)
    {
      Particle.publish("dustvalue", "ppm=" + String(Dust,0)); delay(500);// As the Homebridge Particle plugin does not yet allow gas sensors, you must call it a Humidity sensor
    } // endif ROOM setting "DUST"
...
if (strcmp(OP5_A7, "LASERRCV") == 0)
{
  ...
  if ((millis()-getLaserLastTime) > getLaserInterval) // => It's time to check the LDR signal
  {
    ...
    if (DOORLaserLevel < DOORLigtOnLevel && DOORmovstatus != "Laser monitored doors open") // Door open => DOORLaserLevel LOW and status changed => lights ON
    {
    ...
    if (DOORLaserLevel >= DOORLigtOnLevel && DOORmovstatus != "Laser monitored doors closed") // Doors closed => DOORLaserLevel HIGH and status changed => lights OFF
    {
...
void FunctionKey5() // Default: Turn PIR1 lights ON for the preset time
{
  Displayfreeze = 1; FreezeLastTime = millis(); // Freeze the display
  PIR1Lightson();
  ...
  display.println("5:PIR1 ON"); display.setTextSize(1); display.setCursor(0,20);
...
void FunctionKey6() // Default: Force PIR1 lights OFF
{
  Displayfreeze = 1; FreezeLastTime = millis(); // Freeze the display
  PIR1Lightsoff();
  ...
  display.println("6:PIR1 OFF"); display.setTextSize(1); display.setCursor(0,20);
...
void FunctionKey7() // Default: Force PIR2 lights ON for the preset time
{
  Displayfreeze = 1; FreezeLastTime = millis(); // Freeze the display
  PIR2Lightson();
  ...
  display.println("7:PIR2 ON"); display.setTextSize(1); display.setCursor(0,20);
...
void FunctionKey8() // Default: Force PIR2 lights OFF
{
  Displayfreeze = 1; FreezeLastTime = millis(); // Freeze the display
  PIR2Lightsoff();
  ...
  display.println("8:PIR2 OFF"); display.setTextSize(1); display.setCursor(0,20);
