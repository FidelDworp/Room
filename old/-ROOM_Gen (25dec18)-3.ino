...
int Displaymode = 1; // Initially: Mode1 = Show the fast particle messages on the OLED display.
...
void loop()
{
  ...
  if ((millis()-HomebridgeLastTime) > HomebridgeInterval)
  {
    ...
    if (strcmp(I2C_D0D1, "OLED") == 0)
    {
      Displaymode = 0; // Stop showing the fast particle messages on the display
  ...
  {
    Displaymode = 1; // Start showing the fast particle messages on the display
  }
...
void eventDecoder(const char *event, const char *data) // = Called when an event starting with "Status-" prefix is published...
{
  ...
  if (strcmp(I2C_D0D1, "OLED") == 0 && Displaymode == 1) // Only show the messages if Displaymode = 1
  {
