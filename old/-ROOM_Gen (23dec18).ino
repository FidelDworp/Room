...
// 2) Set suitable "Nightlevel" for this room. (eg: below 150 LUX it is NIGHT)
//
// --------------------------------------
// Functions:
//
...
// - A7 Option2: Laser barrier detects opening door(s): In BandB: A light sensor triggers the cabinet lights ON under a value.
//
...
// - It can display the Particle cloud messages and feedback / instructions of the 16-key touchpad.
//
//
// To do:
...
// OPTION 1: LASER light sensor (LDR)
...
int DimLightLevel = 0;
int FadeTime = 6; // = SECONDS - Change this for slower or faster fade time!
int FadeStep = FadeTime/0.255;
...
bool flashdisplay = 0; // Allow (1) or Stop (0) the fast particle messages to show on the OLED display.
...
void setup()
{
  ...
  // *A7 OPTION 1 = LASER light sensor (LDR) = "LaserLDRpin" (for CABinet lights)
...
void loop()
{
  ...
  if ((millis()-HomebridgeLastTime)>HomebridgeInterval)
  {
    ...
    HomebridgeLastTime = millis(); // Reset timer
  }

  // *A0 & D2: INTERFACE connector:
  if (strcmp(INT_A0, "TOUCH1") == 0)
  {
    // *A0-D2-Single-16touch-keypad
...
// Opgepast: Hieronder 2 wijzigingen voor Lena: Badkamerleds ook met colorpicker laten kiezen...
...
void FunctionKey1()
{
  ...
  display.println("1:PIR1 ON");
  display.setTextSize(1);
  display.setCursor(0,20);
  display.println("Turn PIR1 lights on, whatever the time of the day");
  display.display();
  delay(2000);
}
...
void FunctionKey2()
{
  ...
  display.setTextColor(WHITE);
  display.setCursor(0,0);
  display.println("2:PIR1 OFF");
  display.setTextSize(1);
  display.setCursor(0,20);
  display.println("Turn PIR1 lights off, whatever the time of the day");
  display.display();
  delay(2000);
}
...
void FunctionKey3()
{
  ...
  display.setTextColor(WHITE);
  display.setCursor(0,0);
  display.println("3:PIR2 ON");
  display.setTextSize(1);
  display.setCursor(0,20);
  display.println("Turn PIR2 lights on, whatever the time of the day");
  display.display();
  delay(2000);
}
...
void FunctionKey4()
{
  ...
  display.setTextColor(WHITE);
  display.setCursor(0,0);
  display.println("4:PIR2 OFF");
  display.setTextSize(1);
  display.setCursor(0,20);
  display.println("Turn PIR2 lights off, whatever the time of the day");
  display.display();
  delay(2000);
}
...
void FunctionKey5()
{
  ...
  display.setTextColor(WHITE);
  display.setCursor(0,0);
  display.println("5:STOP!");
  display.setTextSize(1);
  display.setCursor(0,20);
  display.println("Do not show fast particle messages...");
  display.display();
  delay(2000);
}
...
void FunctionKey6()
{
  flashdisplay = 1; // Show fast particle messages
  ...
  display.setTextColor(WHITE);
  display.setCursor(0,0);
  display.println("6:START!");
  display.setTextSize(1);
  display.setCursor(0,20);
  display.println("Show fast particle messages again...");
  display.display();
  delay(2000);
}
...
void FunctionKey7()
{
  ...
  display.setTextSize(1);
  display.setCursor(0,20);
  display.println("Put here some explanation about the key action...");
  display.display();
  delay(2000);
}
...
void readLaser() // Check if laser hits LDR or not => CabinetLaserLevel => Control Cabinet lights
{
  ...
  for(int x = 0 ; x < 20 ; x++)
  {
    CabinetLaserLevel = CabinetLaserLevel + analogRead(A7); // Use A7 directly: You can not create a name for this pin as it is used for 2 functions
  }
...
// CABinet lights: Responding to the LASERRCV signal (A7)
void CABlightsON() // Turn cabinet lights ON
{
...
void eventDecoder(const char *event, const char *data) // = Called when an event starting with "Status-" prefix is published...
{
  ...
  if (strcmp(I2C_D0D1, "OLED") == 0 && flashdisplay)
  {
